...
def ob(n):
    if n == "" or int(n) < 0:
        ...
        return int(n)
def shq(fil, sm, nrsh, sme, na, pa, f, k, pz, kpz, srm, srmd):
    erd = False
    if fil[0] == "":
        #er(6)
        ui.mysignal.emit(6)
    ...
    if erd == 0:
        ...
        #    return er(7)
        ui.mysignal1.emit(1)
        r = sh(binary, sm, nrsh, sme, na, pa, f, k, pz, kpz, srm, srmd)
        ...
        if ui.checkBox_5.checkState() and ui.lineEdit_10.text() == "":
            ...
            f = open(fil[0].split("/")[0]+ui.lineEdit_10.text(), "wb")
            f.write(contents)
...
def dshq(fil,sm, nrsh, sme, na, pa, f, k, pz, kpz, srm, srmd):
    erd = False
    if fil[0] == "":
        ui.mysignal.emit(6)
        erd = True
    if erd == False:
        ...
        if erd == 0:
            ui.mysignal1.emit(2)
            d = dsh(r, sm, nrsh, sme, na, pa, f, k, pz, kpz, srm, srmd)
...
def shqv():
    ...
    if erd:
        ...
        ths = threading.Thread(target=shq, args=(fil, sm, nrsh, sme, na, pa, f, k, pz, kpz, srm, srmd),daemon=True)
        ths.start()
def dshqv():
    erd = True
    try:
...
def save_shif():
    ...
    if file[0] != "":
        ...
        m = [ui.lineEdit.text(), ui.lineEdit_2.text(), ui.lineEdit_3.text(), ui.checkBox.checkState(), ui.checkBox_2.checkState(), ui.lineEdit_4.text(),ui.checkBox_3.checkState(), ui.checkBox_4.checkState(), ui.lineEdit_6.text(), ui.lineEdit_7.text(), ui.lineEdit_5.text()]
        for i in m:
            fi.write(str(ob(i))+"\n")
def open_shif():
